...
y <- SpatialPointsDataFrame(coords = s[, c('lon', 'lat')], 
                            data = data.frame(s[,2] ), proj4string = CRS("+proj=longlat +datum=WGS84"))
class(y)
names(y) <- c('viajes')
...
#leaflet(data = xx) %>% addTiles() %>%
 # addCircles(~long, ~lat,  ~viajes/600, color = 'purple', 
...
p1 <- ggplot(y@data, aes(x = log(Salidas))) +geom_density(color='purple')+
  theme_minimal() + ggtitle('Distribución de los viajes de salida')+
  xlab('número')+ ylab('')
p1
